...
const Navbar = () => {
 ...
 {/* Logo - Left Side */}
<a href="/" className="flex items-center gap-2 transition-all duration-200 hover:opacity-90">
  <img 
    src={assets.Nest} 
    alt="NestHub Logo" 
    className={`
  ...
  />
      ...
      <div className="hidden md:flex items-center gap-8">
        ...
        {user && (
          <div className="flex items-center gap-4 ml-4">
            <button 
              className={`border px-4 py-1 text-sm font-light rounded-full cursor-pointer ${isScrolled ? 'text-black' : 'text-white'} transition-all hover:bg-gray-100 hover:bg-opacity-20`}
      ...
      <div className={`fixed top-0 left-0 w-full h-screen bg-white text-base flex flex-col md:hidden items-center justify-center gap-6 font-medium text-gray-800 transition-all duration-500 ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <button 
          onClick={openSignIn}
          className="bg-black text-white px-8 py-2.5 rounded-full transition-all duration-500 hover:bg-gray-800 mt-4"
        >
          Book Nowuh
        </button>
